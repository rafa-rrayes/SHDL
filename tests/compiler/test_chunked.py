...
A netlist of more than ``_TICK_CHUNK`` words (64 gates each) is string-built
(no .shdl fixture) so the build crosses the chunk boundary: the STRICT_CFLAGS
build (-Werror) proves the SHDLC_NOINLINE/restrict chunk emission compiles
warning-free, and the lockstep run proves chunked tick() semantics match
BaseEval exactly.
"""
...
def test_chunked_build_is_warning_free_and_matches_oracle(builds):
    ...
    assert len(circuit.gates) == N_GATES > _TICK_CHUNK * LANES
...
def _input_only_text(n: int) -> str:
    ...
    chunk functions take the ``in``/``n`` signature (no ``c``). Each gi = XOR(a, b) (i even)
...
def test_n_only_chunk_signature_builds_and_matches_oracle(builds):
    # CCT-17: a chunked circuit whose chunks read no gate outputs emits the
    # `tick_chunk_k(const uint64_t *restrict in, uint64_t *restrict n)`
    # signature without `c`. Build it under STRICT_CFLAGS (-Werror would
    # reject an unused `c` parameter) and lockstep it against BaseEval.
    text = _input_only_text(N_GATES)
    circuit, c_src = compile_text(text)
    assert len(circuit.gates) == N_GATES > _TICK_CHUNK * LANES
    # Proof we are exercising the (in, n) path, not the (c, in, n) path.
    assert "static void tick_chunk_0(const uint64_t *restrict in, uint64_t *restrict n)" in c_src
    assert "restrict c" not in c_src
    assert "tick_chunk_1(inputs, nxt);" in c_src
