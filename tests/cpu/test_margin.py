...
def _trace(cpu, words, limit: int = 400):
    ...
    for _ in range(limit):
        ...
        st = cpu.state()
        out.append((cycles, st["pc"], st["ir"], tuple(st["regs"]),
                    st["z"], st["n"], st["c"], st["v"], st["halted"]))
        if st["halted"]:
            return out
...
def test_doubled_clock_budgets_leave_trace_unchanged(cpu_builds, sr16):
    ...
    slow = SR16(cpu_builds.fresh_path("sr16"),
                t_cap=2 * T_CAP, t_gap=2 * T_GAP, t_settle=2 * T_SETTLE)
