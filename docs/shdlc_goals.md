...
> **Status (PySHDL 1.1.0).** Sections 1–3.2, 5, 6.1, 7 and 8 describe the shipped compiler (`shdlc`, release builds). The **debug build** — the debug ABI (§3.3–3.4), the State Region (§4), debug mode (§6.2) and the `.shdb` file — is **planned and not implemented**: `shdlc` 1.1.0 produces release libraries only. Those sections are kept as the design target and are marked *Planned* where they appear.

---

## 1. The Compiler's Mission

The SHDL compiler (SHDLC) takes a Base SHDL file as input and produces a **compiled simulation artifact** — a shared library (`.so`, `.dylib`, or `.dll`) — that simulates the described circuit. (A planned debug mode will also write a `.shdb` metadata file; see §6.2.)
...
`reset()` returns the simulation to its **power-on state**, not a uniform all-zero state. Every input, output, and gate output is 0 **except** the nets named in the `init` metadata (`base_shdl.md` §4.11, from the source `init` block, `shdl.md` §11.4), which are re-seeded to their declared cycle-0 values, and any pending poke is discarded (the dirty flag is cleared). A circuit with no `init` block therefore resets to all-zero — the common case — while a circuit with `init` (an SR latch that powers up set, a register that powers up at 1) resets to its defined seeded state. `reset()` is idempotent: re-applying it always reproduces this same power-on state, identical to the state of a freshly loaded library (the generated C calls `reset()` from a load-time constructor on GCC-compatible compilers, so the seeds are live before the first call). Constants (`__VCC__`/`__GND__`) take effect on the first evaluation cycle after reset; they are not pre-loaded — the `init` seeds are the only non-zero values present before the first `step()`.
...
**`poke(signal, value)`** — Sets an input by its **user-facing name** from the `ports` metadata (e.g., `"A"`, not `"A_1_"`). Signal names are matched **case-sensitively**, exactly as the language treats identifiers (`shdl.md` §2.2): `"A"` and `"a"` are distinct, and only an exact-case match resolves. Bit 0 of `value` = bit 1 of the port (LSB). The value is **masked modulo 2^width** — any bits at or above the port's width are discarded, so an over-wide value never overflows the port. After a poke, cached outputs are invalidated. Unknown signal names — including the empty string `""` and a **NULL** pointer — take the unknown-signal path: they print `poke: unknown signal "<name>"` to stderr and do nothing, never arming or consuming the dirty flag. NULL is guarded explicitly so a missing name can never cause undefined behavior in the lookup.

**`peek(signal)`** — Reads any input or output by user-facing name (case-sensitive, as `poke`). For outputs, if the circuit has not been evaluated since the last `poke()`, `peek()` triggers exactly one evaluation cycle before returning, so alternating `poke()`/`peek()` always returns consistent results. On a **fresh load** (or after `reset()`), before any `step()`, a `peek()` reads the init-seeded power-on state (`init` metadata applied, §2.4), with the same lazy evaluation cycle applied to outputs. An unknown name prints `peek: unknown signal "<name>"` to stderr and returns 0. Returns the signal's bits packed from the LSB of a `uint64_t`; bits **at or above the port's width are always 0** — the result is zero-extended to 64 bits on every read, symmetric with `poke`'s masking, regardless of any prior over-wide poke or reset.

**`step(cycles)`** — Advances the simulation by exactly `cycles` evaluation cycles. `step(0)` and negative counts do nothing (and leave pending pokes pending).

**`step_settle(cycles)`** — `step(cycles)` with fixed-point early exit: stops as soon as a cycle changes no gate output. With inputs held between cycles, every cycle after such a fixed point recomputes the identical state, so the early exit is **observably identical** to `step(cycles)` — it is purely an optimization, and the caller chooses it per call. Circuits that never settle (oscillators, metastable feedback) never hit the compare and run all `cycles`. Returns the number of cycles actually executed: the 1-based index of the first cycle that changed nothing, or `cycles` if none did. A result below `cycles` therefore proves a fixed point was reached; a result equal to `cycles` does not rule one out (the no-change cycle may have been the last one, and `step_settle(1)` always returns 1). For `cycles >= 1` the result is at least 1 — proving the fixed point costs one evaluation; for `cycles <= 0` it is 0. Every gate is still evaluated on every executed cycle; no gate is ever skipped within a cycle.
...
Two scale limits follow from this ABI and the generated C, and are enforced by the compiler at build time (each is reported as a one-line `shdlc: error: …` message, exit status 1):
...
### 3.3 Debug Build ABI (Planned)

> **Planned — not implemented in PySHDL 1.1.0.** No `shdlc` 1.1.0 library exports these functions. This section is the design target for the debug build.
...
### 3.4 Debug Build Requirements (Planned)
...
## 4. The State Region — Interoperability Foundation (Planned)

> **Planned — not implemented in PySHDL 1.1.0.** Today the generated C keeps its state in separate `static` arrays (`inputs[]`, the double-buffered gate arrays `buf_a[]`/`buf_b[]`, `out_vals[]`) plus a `dirty` flag, with no header, no cycle counter and no external layout description. The function-call ABI (§3.1) is the only interface.
...
These are design targets, not guarantees; actual throughput depends on the circuit, the host and the C compiler.
...
- The C compiler's optimizer: release builds use `-std=c11 -O2 -shared -fPIC -fvisibility=hidden`; `shdlc --dev` swaps `-O2` for `-O0` (much faster C compilation on large circuits, slower simulation). `Circuit(cflags=…)` replaces the flag list entirely.
- CPU-native bitwise operations: each gate maps to `&`, `|`, `^`, or `~`.
- Bit-level parallelism where profitable: many independent same-type gates evaluated per instruction. *Not in 1.1.0:* the released code generator stores one byte (`uint8_t`) per gate and evaluates gates one statement at a time.
- Zero runtime allocation: all state is fixed-size, laid out at compile time.
- Build-time scalability: circuits with more than 1024 gates split the per-gate statements into `noinline` chunk functions of 1024 gates each, so the C compiler's scheduler never sees one giant basic block.
...
The only mode `shdlc` 1.1.0 implements.
...
### 6.2 Debug Mode (Planned)

> **Planned — not implemented in PySHDL 1.1.0.**
...
- Compiles with **clang** (primary), **gcc** (required), **MSVC** (best-effort), as valid C11; compiler-specific extensions only behind `#ifdef`.
  In 1.1.0 the build driver invokes a GCC-style command line (`-std=c11 -O2 -shared -fPIC -fvisibility=hidden`), choosing the compiler from `--cc`/`Circuit(cc=…)`, then `$CC`, then the first of `cc`, `clang`, `gcc` on `PATH`; MSVC can compile the emitted C (`shdlc --no-build --emit-c`) but is not driven automatically, and the load-time `reset()` constructor is GCC/Clang-only.
...
| Block        | Release (1.1.0)   | Debug (planned)                |
|--------------|-------------------|--------------------------------|
| `ports`      | poke/peek codegen (if absent, every raw input/output wire becomes a 1-bit port of the same name) | poke/peek codegen + `.shdb`    |
| `init`       | `reset()` seed table (§2.4) | seed table + `.shdb`   |
| `timing`     | ignored by `shdlc` (the Python driver reads `max_depth`/`has_feedback` for `settle()`) | `.shdb` (drives `settle()`)  |
| `stats`      | ignored | `.shdb`                        |
...
| Build   | Artifacts                                                                  |
|---------|-----------------------------------------------------------------------------|
| Release (shipped) | Shared library: `reset`, `poke`, `peek`, `step`, `step_settle`, `run_batch` — fully optimized |
| Debug (planned) | Shared library: release ABI + `peek_gate`, `peek_gate_prev`, `get_cycle`, `get_num_gates`, `get_gate_info` • `.shdb` JSON (full metadata + State Region layout) |
